...
func TestFirewallCreate(t *testing.T) {
	withTestClient(t, func(config *CmdConfig, tm *tcMocks) {
		...
		config.Config.Set(config.NS, doctl.ArgFirewallName, "firewall")
		config.Config.Set(config.NS, doctl.ArgTagNames, []string{"backend"})
		config.Config.Set(config.NS, doctl.ArgDropletIDs, []string{"1", "2"})
		config.Config.Set(config.NS, doctl.ArgInboundRules, "protocol:icmp protocol:tcp,ports:8000-9000,address:127.0.0.0,address:0::/0,address:::/1")
...
func TestFirewallUpdate(t *testing.T) {
	withTestClient(t, func(config *CmdConfig, tm *tcMocks) {
		...
		config.Args = append(config.Args, fID)
		config.Config.Set(config.NS, doctl.ArgFirewallName, "firewall")
		config.Config.Set(config.NS, doctl.ArgDropletIDs, []string{"1"})
		config.Config.Set(config.NS, doctl.ArgInboundRules, "protocol:tcp,ports:8000-9000,address:127.0.0.0")
		config.Config.Set(config.NS, doctl.ArgOutboundRules, "protocol:tcp,ports:8080,load_balancer_uid:lb-uuid,tag:new-droplets protocol:tcp,ports:80,address:192.168.0.0")
...
func TestFirewallAddTags(t *testing.T) {
	withTestClient(t, func(config *CmdConfig, tm *tcMocks) {
		...
		config.Args = append(config.Args, fID)
		config.Config.Set(config.NS, doctl.ArgTagNames, []string{"frontend", "backend"})
...
func TestFirewallAddRules(t *testing.T) {
	withTestClient(t, func(config *CmdConfig, tm *tcMocks) {
		...
		config.Args = append(config.Args, fID)
		config.Config.Set(config.NS, doctl.ArgInboundRules, "protocol:tcp,ports:80,address:127.0.0.0,address:0.0.0.0/0,address:2604:A880:0002:00D0:0000:0000:32F1:E001 protocol:tcp,ports:8080,tag:backend,droplet_id:1,droplet_id:2,droplet_id:3")
		config.Config.Set(config.NS, doctl.ArgOutboundRules, "protocol:tcp,ports:22,load_balancer_uid:lb-uuid")
...
func TestFirewallRemoveRules(t *testing.T) {
	withTestClient(t, func(config *CmdConfig, tm *tcMocks) {
		...
		config.Args = append(config.Args, fID)
		config.Config.Set(config.NS, doctl.ArgInboundRules, "protocol:tcp,ports:80,address:0.0.0.0/0")
		config.Config.Set(config.NS, doctl.ArgOutboundRules, "protocol:tcp,ports:22,tag:back:end,address:::/0")
